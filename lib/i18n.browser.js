// Generated from translations/*.po. Do not edit directly.
(() => {
  const catalogues = {
  "de-DE": {
    "Annual wheel": "Jahresrad",
    "Annual wheel, home": "Jahresrad, Startseite",
    "Selected year": "Ausgewähltes Jahr",
    "Previous year": "Vorheriges Jahr",
    "Next year": "Nächstes Jahr",
    "Open account": "Konto öffnen",
    "Annual plan": "Jahresplan",
    "Filter layers": "Ebenen filtern",
    "Key dates": "Wichtige Termine",
    "{count} deadlines": "{count} Fristen",
    "Adjust date": "Datum anpassen",
    "Display date": "Anzeigedatum",
    "Date at the top of the wheel": "Datum oben im Jahresrad",
    "Within {count} days": "Innerhalb von {count} Tagen",
    "{organisation} · key dates": "{organisation} · wichtige Termine",
    "Assumptions": "Annahmen",
    "View all deadlines": "Alle Fristen anzeigen",
    "Today": "Heute",
    "Zoom annual wheel": "Jahresrad zoomen",
    "Zoom out": "Verkleinern",
    "Zoom in": "Vergrößern",
    "Recurring rhythm": "Wiederkehrender Rhythmus",
    "a new cycle every year": "ein neuer Zyklus jedes Jahr",
    "Drag the wheel to browse the year · select a marker to see the deadline": "Ziehen Sie das Rad, um durch das Jahr zu blättern · wählen Sie eine Markierung für die Frist",
    "More about {title}": "Mehr über {title}",
    "Tentative": "Vorläufig",
    "Set": "Festgelegt",
    "Anchor date": "Ankerdatum",
    "Set date": "Festgelegtes Datum",
    "Depends on anchor": "Hängt vom Anker ab",
    "Depends on set anchor": "Hängt vom festgelegten Anker ab",
    "{count} course days": "{count} Kurstage",
    "Rendering mode": "Darstellungsmodus",
    "Year by year": "Jahr für Jahr",
    "Continuous": "Kontinuierlich",
    "Continuous mode: dates enter at the top and leave beneath the lower cut.": "Im kontinuierlichen Modus erscheinen Daten oben und verlassen das Rad unterhalb des unteren Schnitts."
  },
  "en-GB": {
    "Annual wheel": "Annual wheel",
    "Annual wheel, home": "Annual wheel, home",
    "Selected year": "Selected year",
    "Previous year": "Previous year",
    "Next year": "Next year",
    "Open account": "Open account",
    "Annual plan": "Annual plan",
    "Filter layers": "Filter layers",
    "Key dates": "Key dates",
    "{count} deadlines": "{count} deadlines",
    "Adjust date": "Adjust date",
    "Display date": "Display date",
    "Date at the top of the wheel": "Date at the top of the wheel",
    "Within {count} days": "Within {count} days",
    "{organisation} · key dates": "{organisation} · key dates",
    "Assumptions": "Assumptions",
    "View all deadlines": "View all deadlines",
    "Today": "Today",
    "Zoom annual wheel": "Zoom annual wheel",
    "Zoom out": "Zoom out",
    "Zoom in": "Zoom in",
    "Recurring rhythm": "Recurring rhythm",
    "a new cycle every year": "a new cycle every year",
    "Drag the wheel to browse the year · select a marker to see the deadline": "Drag the wheel to browse the year · select a marker to see the deadline",
    "More about {title}": "More about {title}",
    "Tentative": "Tentative",
    "Set": "Set",
    "Anchor date": "Anchor date",
    "Set date": "Set date",
    "Depends on anchor": "Depends on anchor",
    "Depends on set anchor": "Depends on set anchor",
    "{count} course days": "{count} course days",
    "Rendering mode": "Rendering mode",
    "Year by year": "Year by year",
    "Continuous": "Continuous",
    "Continuous mode: dates enter at the top and leave beneath the lower cut.": "Continuous mode: dates enter at the top and leave beneath the lower cut."
  },
  "es-ES": {
    "Annual wheel": "Rueda anual",
    "Annual wheel, home": "Rueda anual, inicio",
    "Selected year": "Año seleccionado",
    "Previous year": "Año anterior",
    "Next year": "Año siguiente",
    "Open account": "Abrir cuenta",
    "Annual plan": "Plan anual",
    "Filter layers": "Filtrar capas",
    "Key dates": "Fechas clave",
    "{count} deadlines": "{count} plazos",
    "Adjust date": "Ajustar fecha",
    "Display date": "Fecha de visualización",
    "Date at the top of the wheel": "Fecha en la parte superior de la rueda",
    "Within {count} days": "Dentro de {count} días",
    "{organisation} · key dates": "{organisation} · fechas clave",
    "Assumptions": "Supuestos",
    "View all deadlines": "Ver todos los plazos",
    "Today": "Hoy",
    "Zoom annual wheel": "Ampliar rueda anual",
    "Zoom out": "Alejar",
    "Zoom in": "Acercar",
    "Recurring rhythm": "Ritmo recurrente",
    "a new cycle every year": "un nuevo ciclo cada año",
    "Drag the wheel to browse the year · select a marker to see the deadline": "Arrastre la rueda para explorar el año · seleccione un marcador para ver el plazo",
    "More about {title}": "Más sobre {title}",
    "Tentative": "Provisional",
    "Set": "Fijado",
    "Anchor date": "Fecha de referencia",
    "Set date": "Fecha fijada",
    "Depends on anchor": "Depende de la referencia",
    "Depends on set anchor": "Depende de la referencia fijada",
    "{count} course days": "{count} días de curso",
    "Rendering mode": "Modo de visualización",
    "Year by year": "Año por año",
    "Continuous": "Continuo",
    "Continuous mode: dates enter at the top and leave beneath the lower cut.": "Modo continuo: las fechas entran por la parte superior y salen por debajo del corte inferior."
  },
  "nb-NO": {
    "Annual wheel": "Årshjul",
    "Annual wheel, home": "Årshjul, forsiden",
    "Selected year": "Valgt år",
    "Previous year": "Forrige år",
    "Next year": "Neste år",
    "Open account": "Åpne konto",
    "Annual plan": "Sesongplan",
    "Filter layers": "Filtrer lag",
    "Key dates": "Nøkkeldatoer",
    "{count} deadlines": "{count} frister",
    "Adjust date": "Endre dato",
    "Display date": "Visningsdato",
    "Date at the top of the wheel": "Datoen øverst i hjulet",
    "Within {count} days": "Innenfor {count} dager",
    "{organisation} · key dates": "{organisation} · nøkkeldatoer",
    "Assumptions": "Forutsetninger",
    "View all deadlines": "Se alle frister",
    "Today": "I dag",
    "Zoom annual wheel": "Zoom i årshjulet",
    "Zoom out": "Zoom ut",
    "Zoom in": "Zoom inn",
    "Recurring rhythm": "Gjentakende rytme",
    "a new cycle every year": "en ny runde hvert år",
    "Drag the wheel to browse the year · select a marker to see the deadline": "Dra hjulet for å bla i året · trykk på en markør for å se fristen",
    "More about {title}": "Mer om {title}",
    "Tentative": "Tentativ",
    "Set": "Fastsatt",
    "Anchor date": "Ankerdato",
    "Set date": "Fastsatt dato",
    "Depends on anchor": "Avhenger av anker",
    "Depends on set anchor": "Avhenger av fastsatt anker",
    "{count} course days": "{count} kursdager",
    "Rendering mode": "Visningsmodus",
    "Year by year": "År for år",
    "Continuous": "Kontinuerlig",
    "Continuous mode: dates enter at the top and leave beneath the lower cut.": "Kontinuerlig modus: datoer kommer inn øverst og forsvinner under snittet nederst."
  }
};
  const resolveLocale = requested => {
    if (catalogues[requested]) return requested;
    const language = requested.split('-')[0];
    return Object.keys(catalogues).find(locale => locale.startsWith(language + '-'));
  };
  const translate = (locale, id, values = {}) => (catalogues[locale][id] || id).replace(/\{(\w+)\}/g, (_, name) => values[name] ?? '{' + name + '}');
  window.annualWheelI18n = { resolveLocale, translate };
})();
